...
for i in range(student_count):
    org_grades.append(float(input()))

for i in range(student_count):
    new_grades.append(float(input()))

alt_grades = [i for i in org_grades]

alt_count = 0

for i in range(len(new_grades)):
    if new_grades[i] == 10.00 and org_grades[i] != 10.00:
        alt_grades[i] += 2
        alt_count += 1
        if alt_grades[i] > 10:
            alt_grades[i] = 10.00

print("NOTAS ALTERADAS:", alt_count)
for i in range(len(org_grades)):
    if org_grades[i] == alt_grades[i]:
        char = "-"
    else:
        char = "*"

    print(f"{char}({i+1:03}) original: {org_grades[i]:05.2f} | final: {alt_grades[i]:05.2f}")