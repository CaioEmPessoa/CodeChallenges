...
FILENAME='DrawLines-3ds'
FILETYPE='3dsx'
DS_IP='172.20.23.121' # 192.168.XXX.XXX (without port) (needed if on original hardware)
CITRA_PATH='' # (needed if on citra)
CITRA_LOG_PATH=''
...
if [ "$run_arg" != "false" ]; then
    ...
    if [ "$native_arg" == "true" ]; then
        ...
        3dslink $FILENAME.$FILETYPE -a $DS_IP
    else
