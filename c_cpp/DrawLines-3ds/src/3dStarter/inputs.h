...
void checkTouchClick() // check and execute if touch is on one of squares.
{
	...
	for (int i = 0; i < stored_touch_count; i++)
	{
		...
		int x0 = item.xywh[0];
		int x1 = x0 + item.xywh[2]; // sum cause things are drawn to the right
		int y0 = item.xywh[1];
		int y1 = y0 + item.xywh[3]; // decrease cause things are drawn downwards

		int posX = deconvertPosInScreen('w', touch.px, BOTTOM); // touch and this variable is stored globally
                                                    // TODO: why does decrease the value ?
		int posY = deconvertPosInScreen('h', touch.py, BOTTOM);
